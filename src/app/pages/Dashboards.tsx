...
export default function Dashboards({ id_magasin, user_id, savedFavoris = [], onNavigate, onSetQuestion, onDeleteFavori, onEditFavori, onEditFavoriInQuestions, refreshTrigger }: DashboardsProps) {
  ...
  const executeInCell = async (cellIdx: number, titre: string, question: string) => {
    ...
    try {
      ...
      // Appliquer le type de viz du favori s'il en a un
      const favVizMap: Record<string, string> = { barres: "bar", ligne: "line", camembert: "pie", kpi: "kpi", tableau: "table" };
      if (fav?.viz && favVizMap[fav.viz]) {
        result.viz_config = { ...result.viz_config, type_viz: favVizMap[fav.viz] };
      }
      setDashboards(prev => prev.map(d => {
        if (d.id !== activeDb) return d;
        const cells = [...d.cells];
          ...
          >
            ...
            <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 6 }}>
              <button onClick={() => {
                ...
                style={{ padding: "8px 0", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", border: "2px solid #3AA48A", background: "#fff", color: "#3AA48A" }}
              >Modifier</button>
              <button onClick={() => {
              ...
              <button onClick={() => setEditingFav(null)}
                style={{ padding: "8px 0", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", border: "1px solid #d0e8e2", background: "#fff", color: "#4a7068" }}
              >Annuler</button>
              <button onClick={() => {
                ...
                style={{ padding: "8px 0", borderRadius: 8, fontSize: 12, fontWeight: 600, cursor: "pointer", fontFamily: "inherit", border: "none", background: "#3AA48A", color: "#fff" }}
              >Enregistrer</button>
            </div>
          </div>
        </div>
